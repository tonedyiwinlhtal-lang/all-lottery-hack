...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!fetchResponse.data || !fetchResponse.data.data || !Array.isArray(fetchResponse.data.data.list)) {
      throw new Error("API returned invalid data format, likely an expired token. Falling back to mock.");
    }
